...
- [docs/QUICKSTART-v2.ja.md](docs/QUICKSTART-v2.ja.md)
- [docs/V2-MIGRATION-CHECKLIST.md](docs/V2-MIGRATION-CHECKLIST.md)
- [docs/MIGRATION-v1-to-v2.md](docs/MIGRATION-v1-to-v2.md)
- 先に依存関係をインストール: `python3 -m pip install -r backend/requirements-dev.txt`
...
`backend` + `frontend`（`./start.sh`）の v1 系は新規利用では legacy/deprecated 扱いです。
...
CSV内容を貼り付けてデバイスをインポートし検証します。アプリケーションは各デバイスに対して軽量な接続テスト（認証のみ）を実行し、インポート中は検証の進捗を表示します。
